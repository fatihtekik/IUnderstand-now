...
def connect_to_db():
    try:
        conn = psycopg.connect(
            dbname="postgres",
            user="postgres",
            password="Aidana2007",  # Замените на ваш пароль базы данных
            host="localhost",
            port="5432"
        )
        return conn
...
def start_message(message):
    ...
    bot.send_message(
        message.chat.id,
        "Добро пожаловать в EVALIX, бот по учебной части!\nВведите ваш логин.",
...
def choose_group(message):
    ...
    if "group_id" in user_data[message.chat.id]:
        ...
        bot.send_message(
            message.chat.id,
            f'Вы выбрали группу {user_data[message.chat.id]["group_id"]}. Теперь выберите день недели:',
            reply_markup=markup
        )
    else:
...
def choose_day(call):
    ...
    bot.send_message(
        ...
        f'Вы выбрали группу {group_id}. Теперь выберите день недели:',
        reply_markup=markup
    )
...
def show_day_schedule(call):
    ...
    group_id = user_data.get(call.message.chat.id, {}).get("group_id")
    if not group_id:
    ...
    if group_id in schedules:
        ...
        if schedule:
            ...
            bot.send_message(
                call.message.chat.id,
                f'Расписание на {day} для группы {group_id}:\n{schedule_text}'
            )
        else:
...
@bot.callback_query_handler(func=lambda call: call.data in ["docs_add", "docs_show"])
        ...
        if docum:
            for doc in docum:
                ...
                try:
                    ...
                    os.remove(doc['file_path'])  # Удаляем файл после отправки
                except FileNotFoundError:
                    bot.send_message(call.message.chat.id, f"Файл {doc['file_name']} не найден.")
        else:
...
def handle_file(message):
    if user_data.get(message.chat.id, {}).get("step") == "awaiting_document":
        if message.document:
            ...
            save_path = os.path.join("documents", message.document.file_name)  # сохраняем файл с его исходным именем
...
def save_file_db(stud, file_name, file_path):
    ...
    try:
        ...
        print("Ошибка сохранения документа в базе данных:", e)
    finally:
        cursor.close()
        conn.close()
...
def reiting(message):
    ...
    cursor.execute(
        '''SELECT students.id_student, AVG(ocenki.ocenka) as avg_grade, 
                  SUM(CASE WHEN ocenki.ocenka = 0 THEN 1 ELSE 0 END) as count_zeros
           FROM ocenki 
           JOIN students ON students.id_student = ocenki.id_student  
